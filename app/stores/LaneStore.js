...
class LaneStore {
  ...
  attachToLane({ laneId, noteId }) {
    ...
    if (!noteId) {
      ...
      fnoteId = NoteStore.getState().notes.slice(-1)[0].id;
    }

    this.removeNote(fnoteId);

    const lanes = this.lanes.map((lane) => {
      if (lane.id === laneId) {
        if (lane.notes.indexOf(fnoteId) === -1) {
          lane.notes.push(fnoteId);
        } else {
  ...
  move({ sourceId, targetId }) {
    ...
    const sourceNoteIndex = sourceLane.notes.indexOf(sourceId),
          targetNoteIndex = targetLane.notes.indexOf(targetId);
